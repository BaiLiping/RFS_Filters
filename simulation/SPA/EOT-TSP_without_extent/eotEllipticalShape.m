...
for step = 1:numSteps
    ...
    for outer = 1:numOuterIterations
        ...
        for measurement = numMeasurements:-1:1 % measurement l
            inputDA = ones(2,numLegacy);
            
            for target = 1:numLegacy % numLegacy K underscore target: k
                
                if(outer == 1)
                    % initiate the definintion of weight
            ...
            for target = numMeasurements:-1:measurement
                ...
                if(any(target==newIndexes))
                    ...
                    if(target == measurement)
                        % what is the equation related to this expression?
