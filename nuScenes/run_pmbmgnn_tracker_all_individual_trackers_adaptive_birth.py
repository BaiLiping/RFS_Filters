...
from utils.utils import BBox, fuse_duplicated_track,predict_for_all,incorporate_track,visualize_duplicated_detection,iou3d,compute_duplicated_detection,fill_frame_result,gen_measurement_all, nms, nu_array2mot_bbox,create_scene_folder_name,create_experiment_folder, initiate_submission_file, gen_measurement_of_this_class, initiate_classification_submission_file, readout_parameters
from trackers.PMBMGNN import PMBMGNN_Filter_Point_Target_single_class_adaptive_birth as pmbmgnn_tracker
...
def parse_args():
    ...
    parser.add_argument('--detection_file',default='/home/zhubinglab/Desktop/val_result_with_classification.json', help='directory for the inference file')
        ...
        for frame_idx, frame_token in enumerate(ordered_frames):
            ...
            for classification_index, classification in enumerate(classifications):
                if frame_idx==0:
                    ...
                    Z_k_all_fused=fuse_duplicated_track(Z_k_all)
                    Z_k_all_rectified=Z_k_all_fused
                    Z_k=[]
                ...
                if frame_idx == 0:
                    ...
                    filter_predicted_dict[classification]=filter_predicted
                # update
                filter_updated = filters[classification_index].update(frame_idx, ego_info[str(frame_idx)], Z_k, filter_predicted, confidence_score,giou_gating)
                ...
                for track_idx, track in enumerate(frame_result_without_duplication_temp):
                    if len(track['duplicated_detection'])>0:
                        ...
                        if normalized_tracking_scores[highest_class_index]>0.8:
                            ...
                            frame_result_without_duplication.append(track)
                            for z_1 in track['duplicated_detection']:
                                if z_1['tracking_id']!=track['tracking_id']:
                ...
                duplicated_delete=[]
                skip=[]
                for track1_idx, track1 in enumerate(frame_result_without_duplication):
                    if track1_idx not in skip:
                        for track2_idx, track2 in enumerate(frame_result_without_duplication):
                            if track1['tracking_id']==track2['tracking_id'] and track1_idx!=track2_idx:
                                skip.append(track1)
                                duplicated_delete.append(track2)

                for deleted_track in duplicated_delete:
                    print('deleted {}'.format(deleted_track['tracking_name']))
                    frame_result_without_duplication.remove(deleted_track)
                '''
