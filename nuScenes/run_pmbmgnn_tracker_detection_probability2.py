...
from utils.utils import nu_array2mot_bbox, iou3d, create_scene_folder_name,nms, incorporate_track, predict_for_all, create_experiment_folder, initiate_submission_file, gen_measurement_of_this_class, initiate_classification_submission_file, predict_for_all, readout_parameters
...
def parse_args():
    ...
    parser.add_argument('--detection_file',default='/home/zhubinglab/Desktop/val_results.json', help='directory for the inference file')
...
def main(out_file_directory_for_this_experiment):
    ...
    register_for_gt_classifications=[np.zeros(7) for i in range(7)]
    count_for_gt_classifications=np.zeros(7)
        ...
        for frame_idx, frame_token in enumerate(ordered_frames):
            ...
            for classification_index, classification in enumerate(classifications):
                ...
                if frame_idx == 0:
                    ...
                    #    else:
                    #        print(classifications[np.argmax(classification_probability)])
                    #        print(classification)
                    Z_k_with_track=copy.deepcopy(Z_k)
                    filter_predicted = filters[classification_index].predict_initial_step(Z_k_with_track, birth_rate)
                    filter_predicted_list[classification]=filter_predicted
                    ...
                    #Z_k_with_track=incorporate_track(Z_k_predict, Z_k, frame_idx)
                    #result_indexes = nms(Z_k_temp, threshold=0.1)
                    #Z_k_with_track=[]
                    #for idx in result_indexes:
                    #    Z_k_with_track.append(Z_k_temp[idx])
                    Z_k_with_track=copy.deepcopy(Z_k)
                    filter_predicted = filters[classification_index].predict(ego_info[str(frame_idx)],time_lag,filter_pruned_list[classification], Z_k_with_track, birth_rate)
                    filter_predicted_list[classification]=filter_predicted
                # update
                filter_updated = filters[classification_index].update(Z_k_with_track,  filter_predicted_list[classification], confidence_score,-0.5)
                filter_updated_list[classification]=filter_updated
                # state extraction
                ...
                filter_pruned_list[classification]=filter_pruned
            ...
            for gt_bbox_idx, gt_bbox in enumerate(ground_truth_bboxes_for_this_frame):
                gt_name=ground_truth_type_for_this_frame[gt_bbox_idx]
                count_for_gt_classifications[classifications_index[gt_name]]+=1
                gt_bbox_formated={}
                gt_bbox_formated['translation']=[gt_bbox[0], gt_bbox[1], gt_bbox[2]]
                gt_bbox_formated['size']=[gt_bbox[3], gt_bbox[4], gt_bbox[5]]
                gt_bbox_formated['rotation']=[gt_bbox[6], gt_bbox[7], gt_bbox[8], gt_bbox[9]]

                for result_bbox in frame_result:
                    iou_result=iou3d(nu_array2mot_bbox(gt_bbox_formated), nu_array2mot_bbox(result_bbox))[0]
                    if iou_result>0.5:
                        register_for_gt_classifications[classifications_index[gt_name]][classifications_index[result_bbox['tracking_name']]]+=1
    ...
    #print out result
    for classification_index, classification in enumerate(classifications):
        number_of_gt_this_class=count_for_gt_classifications[classification_index]
        print('there are {} gt for {}'.format(number_of_gt_this_class,classification))
        percentage_record=[]
        for classification_index2, classification2 in enumerate(classifications):
            detection_of_this_class = register_for_gt_classifications[classification_index]
            percentage=np.round(detection_of_this_class[classification_index2]/number_of_gt_this_class,2)
            percentage_record.append(percentage)
            print('{} of {} is detected as {}'.format(percentage,classification,classification2))
        miss_detected=1-np.round(np.sum(percentage_record),2)
        print('{} of {} is miss detected'.format(miss_detected,classification))
