            ...
            for single_target_hypothesis_index_from_previous_frame in range(number_of_single_target_hypotheses_from_previous_frame):
                ...
                for measurement_index in range(number_of_measurements_from_current_frame): # starting from m_1, since m_0 means missed detection
                    ...
                    if within_gating: 
                        ...
                        #if classificationB_single_target_hypothesis_predicted=='truck':
                            #classificationB_single_target_hypothesis_updated='truck'
                        #else:
                            #classificationB_single_target_hypothesis_updated = Z_k[measurement_index]['detection_name']
                        #classificationB_single_target_hypothesis_updated = Z_k[measurement_index]['detection_name']
                        if classificationB_single_target_hypothesis_predicted=='pedestrian':
                            classificationB_single_target_hypothesis_updated = Z_k[measurement_index]['detection_name']
                        else:
                            if len(Z_k[measurement_index]['duplicated_detection'])>0:
                                probabilities=[]
                                classes=[]
                                probabilities.append(Z_k[measurement_index]['detection_score'])
                                classes.append(Z_k[measurement_index]['detection_name'])
                                for z in Z_k[measurement_index]['duplicated_detection']:
                                ...
                                if classificationB_single_target_hypothesis_predicted in classes:
                                    index_for_previous_track_classification=classes.index(classificationB_single_target_hypothesis_predicted)
                                    for index,probability in enumerate(probabilities):
                                        if index!=index_for_previous_track_classification:
                                            probabilities[index]=probability*0.5
                                '''
                                
                                # normalize
                                #print('detection scores {}'.format(probabilities))
                                #print('detection names {}'.format(classes))
                                #print('track name {}'.format(classificationB_single_target_hypothesis_predicted))
                                #normalized_probabilities=probabilities/np.sum(probabilities)
                                highest_class_index=np.argmax(probabilities)
                                highest_class=classes[highest_class_index]
                                classificationB_single_target_hypothesis_updated=highest_class
                            else:
                                classificationB_single_target_hypothesis_updated = Z_k[measurement_index]['detection_name']
