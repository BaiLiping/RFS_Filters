...
def parse_args():
    ...
    parser.add_argument('--detection_file',default='/home/bailiping/Desktop/val_results_without_duplication.json', help='directory for the inference file')
    ...
    for scene_idx in range(len(list(orderedframe.keys()))):
        ...
        for frame_idx, frame_token in enumerate(ordered_frames):
            ...
            for classification_index, classification in enumerate(classifications):
                Z_k_without_velocity=[]
                if len(Z_k_original)>0:
                    for z in Z_k_original:
                        if z['detection_name']==classification:
                            Z_k_without_velocity.append(z)
                else:
                    Z_k_without_velocity=Z_k_original
                
                if classification=='car' or classification=='bus' or classification=='truck' or classification=='trailer':    
                    Z_k=[]
                    for z in Z_k_without_velocity:
                        position=z['translation']
                        size=z['size']
                        rotation=Quaternion(z['rotation'])
                        box = Box(position,size,rotation)
                    ...
                    Z_k=copy.deepcopy(Z_k_without_velocity)   
                '''
                else:
            ...
            if args.plot_result:
                if frame_idx<2:
                    render_tracker_result(0,ground_truth_bboxes_for_this_frame,ground_truth_type_for_this_frame,frame_result_without_duplication,frame_token,nsweeps=1,out_path=out_file_directory_for_this_scene+'/{}.png'.format(frame_idx),bird_eye_view_with_map=False,verbose=False)
