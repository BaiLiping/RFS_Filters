...
def parse_args():
    ...
    parser.add_argument('--dataset_folder',default='/home/zhubinglab/Desktop/mmdetection3d/data/nuscenes')
    ...
    for data_version in dataset_versions:
        ...
        for scene_index, scene_info in enumerate(scenes):
            ...
            while True:
                ...
                for radar_position in radar_positions:
                    pc, times = RadarPointCloud.from_file_multisweep(nusc, frame_data, radar_position, 'LIDAR_TOP', nsweeps=10)
                    radar_token=frame_data['data'][radar_position]
                    radar_data = nusc.get('sample_data', radar_token)
                    radar_path=radar_data['filename']
                    calibrated_sensor_token=nusc.get('sample_data', radar_token)['calibrated_sensor_token']
                
                    #frame_data['data']['RADAR_FRONT']
                    radar_cs_record = nusc.get('calibrated_sensor', calibrated_sensor_token)

                    aggregated_info[set_info]['radar_token'][radar_position][scene_token].append(radar_token)
                    aggregated_info[set_info]['radar_path'][radar_position][scene_token].append(radar_path)
                    aggregated_info[set_info]['radar_calibration_token'][radar_position][scene_token].append(calibrated_sensor_token)
                    velocities = pc.points[8:10, :]  # Compensated velocity
                    velocities = np.vstack((velocities, np.zeros(pc.points.shape[1])))
                    velocities = np.dot(Quaternion(radar_cs_record['rotation']).rotation_matrix, velocities)
                    velocities = np.dot(Quaternion(calib_pose['rotation']).rotation_matrix.T, velocities)
                    velocities[2, :] = np.zeros(pc.points.shape[1])
                    pc.points[8:10,:]=velocities[:2,:]
                    if radar_position=='RADAR_FRONT':
                        point_total=deepcopy(pc.points)
                    else:
                        point_total = np.hstack((point_total, pc.points))
                aggregated_info[set_info]['radar_points'][scene_token][str(frame_index)]=point_total
