...
def box3d_iou(corners1, corners2, criterion='union'):
    ...
    rect1 = [(corners1[i,0], corners1[i,2]) for i in range(3)]
    rect2 = [(corners2[i,0], corners2[i,2]) for i in range(3)] 
    area1 = poly_area(np.array(rect1)[:,0], np.array(rect1)[:,1])
    area2 = poly_area(np.array(rect2)[:,0], np.array(rect2)[:,1])
    inter, inter_area = convex_hull_intersection(rect1, rect2)
    
    #ymax = min(corners1[0,1], corners2[0,1])
    #ymin = max(corners1[43,1], corners2[4,1])
    #inter_vol = inter_area * max(0.0, ymax-ymin)
    #vol1 = box3d_vol(corners1)
    #vol2 = box3d_vol(corners2)
    union_2d=area1+area2-inter_area

    iou_2d = inter_area/union_2d
