...
from trackers.SPA import SPA_filter as spa_tracker
from trackers.SPA import util as spa_ulti
...
def main(classification,out_file_directory_for_this_experiment):
    ...
    filter_model = spa_ulti.gen_filter_model(classification, clutter_rate, P_d, P_s,poi_thr)
    ...
    for scene_idx in range(len(list(orderedframe.keys()))):
        ...
        spa_filter = spa_tracker.PMB_Filter(filter_model, 'constant velocity', 'Linear Kalman Filter', classification)
        Z_list = []
        id_max=0
        initialize=False
        ...
        for frame_idx, frame_token in enumerate(ordered_frames):
            ...
            if initialize==False:  # For the fisrt frame, there are only new birth targets rather than surviving targets thus we call seperate function.
                ...
                filter_predicted = spa_filter.predict_for_initial_step(Z_k, birth_rate)
                initialize=True
                filter_updated, id_max = spa_filter.update(Z_k, filter_predicted, Z_list,id_max,confidence_score,bernoulli_gating) #Eq. 20 of [2]
                # loopy belief propogation
                pupd,pnew = spa_filter.loopy_belief_propogation(Z_k, filter_updated)
                r,filter_updated, Z_list= spa_filter.tomb(pupd,pnew,filter_updated,r_threshold)
                ...
                filter_predicted, Z_list= spa_filter.predict(Z_k,filter_updated, Z_list, birth_rate,time_lag)  
                filter_updated, id_max = spa_filter.update(Z_k, filter_predicted, Z_list,id_max,confidence_score,bernoulli_gating)
                # loopy belief propogation
                pupd,pnew = spa_filter.loopy_belief_propogation(Z_k, filter_updated)
                r,filter_updated, Z_list= spa_filter.tomb(pupd,pnew,filter_updated,r_threshold)
                estimatedStates_for_this_classification=[]
